...
function getParams(phone_input){
	...
	if(pass==1){
		...
		phone_number_1=contact.phone_number;
	}
	else if (pass==2){
		...
		phone_number_2=contact.phone_number;
	}
	else{
		contact.name=urlParams.get('name_1');
...
function getConversationId(contact,key){
	...
	request.open('GET', 'https://private-a1d8aa-textline.apiary-proxy.com/api/customers.json?phone_number='+contact.phone_number+'&access_token='+key); /* a941205863 */
...
function storeContactData(contact,key,uuid){
	...
	if(typeof uuid==='undefined'){
		request.open('POST', 'https://private-a1d8aa-textline.apiary-proxy.com/api/customers.json?access_token='+key);
	}
	else{
		request.open('PUT', 'https://private-a1d8aa-textline.apiary-proxy.com/api/customer/'+uuid+'.json?access_token='+key);
	}
